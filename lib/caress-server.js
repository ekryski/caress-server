...
function Caress(host, port, options) {
  ...
  tuioUdpSocket.on('message', function(packet, rinfo) {
    var b = new Buffer(packet);
    fs.writeFile(__dirname + '/../tmp', b.toHex() + '\n', function(err){
      if (err) console.log(err);
      else console.log(packet.toString());
    });
  ...
  tuioUdpSocket.on('error', function(error){
    //TODO: Do something with this error. Should probably emit it as well.
